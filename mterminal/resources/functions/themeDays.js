...
Våffeldagen
Tolkien reading day
Internationella solidaritetsdagen för frihetsberövad eller förvunnen personal
26 Mars
Dialersystemens dag
27 Mars
Världsteaterdagen
...
31 Mars
Världsbackupdagen
Internationella dagen för synliggörande av transpersoner
1 April
Skojardagen
2 April
Världsautismdagen
...
7 April
Internationella dagen för reflektion av folkmordet i Rwanda
Världshälsodagen
8 April
...
24 April
Försöksdjurens dag
Internationella dagen för multilateralism och diplomati för fred
25 April
Världsmalariadagen
Mamatummy Day
26 April
...
Promenadens dag
Alien-dagen
Internationella dagen till minnet av Tjernobylkatastrofen
27 April
Världstapirdagen
28 April
Arbetsmiljödagen
...
8 Maj
Hågkomst och försoning för de som omkom under andra världskriget
...
Alla vänners dag
Barfotadagen
Internationella dagen för kampen mot illegalt, orapporterat och oreglerat fiske
6 Juni
Dagen-D
Fruktens dag
Färskpotatisens dag
...
13 Juni
Internationella dagen för medvetenhet om albinism
14 Juni
Internationella blodgivardagen
15 Juni
Chaufförens dag
Världsdagen om våld och övergrepp mot äldre
16 Juni
Bloomsday
...
27 Juni
Internationella dagen för små- och medelstora företag
30 Juni
Pilsnerkorvens dag
...
Hudvårdens dag
9 Juli
Internationella dagen för oskadliggörande av handeldvapen
10 Juli
Hoya - Porslinsblommans dag
11 Juli
FN:s befolkningsdag
Internationella råkostdagen
Internationella minnesdagen för Srebrenica
12 Juli
Fulhetens dag
13 Juli
Paltdagen
...
Durspelets dag
Internationella dagen till minnet av terrorismens offer
Groggens dag
22 Augusti
Internationella müslidagen
Internationella minnesdagen för människor som fallit offer för våldsdåd relaterade till religion...
23 Augusti
...
30 Augusti
Internationella dagen för offren för påtvingade försvinnanden
31 Augusti
...
2 November
De dödas dag
Internationella dagen för att för att upphäva straffrihet för brott mot journalister
4 November
No hate day
5 November
Guy Fawkes Day
Kaninköttets dag
Internationella dagen för tsunamikännedom
6 November
Gustav Adolfsdagen
...
Miniatyrrävens dag
Globala lunginflammationsdagen
13 November
Smörgåstårtans dag
14 November
Världsdiabetesdagen
...
function themeDay(){
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const match1 = line.match(/^(\d{1,2} \w{3,})/);
        const match2 = line.match(/^(.*)/);

        if (match1) {
            dateParts = match1[1].split(' ');
            day = parseInt(dateParts[0], 10);
            month = dateParts[1];

        } else if (match2) {
            event = lines[i];
            events.push({ day, month, event});
        }
    };
    ...
    let now = new Date();
    let nowDayNumber = now.getDate();
    let nowMonthNumber = now.getMonth();
    console.log('Day number: ' + nowDayNumber);
    console.log('now: ' + now);
    //console.log('Month number: ' + nowMonthNumber);

    const monthArray = ['Januari', 'Februari', 'Mars', 'April', 'Maj', 'Juni', 'Juli', 'Augusti', 'September', 'Oktober', 'November', 'December'];

    let nowMonthName = monthArray[nowMonthNumber];
    console.log('Month name: ' + nowMonthName);
    //console.log(monthArray);

    // Get the parent element of id 'themeDay'
    const parentElement = document.querySelector('#themeDay');

    // Get all the children elements if there are any and then remove them
    if (parentElement.hasChildNodes()) {

        // Select all child elements
        const children = parentElement.querySelectorAll('div');
        console.log('Found some child div elements');

        // Remove all children 'div' elements
        children.forEach(child => {
            child.remove();
        });
    } else {
        console.log('There are no child elements');
    };

    let numThemedays = 0;

    // Loop through themeDay array and add theme days as child 'div' elements
    for (let i = 0; i < events.length; i++) {
        if (nowDayNumber === events[i].day && nowMonthName === events[i].month) {

            // Set new element
            const newElement = document.createElement('div');
            newElement.textContent = events[i].event;

            // Add each themeDay as a new 'div' element
            parentElement.appendChild(newElement);
            numThemedays = numThemedays + 1;
        };
    };

    console.log(numThemedays);
    ...
    if (numThemedays === 0) {
        ...
        // Set new element
        const newElement = document.createElement('div');
        newElement.textContent = 'Ingen temadag idag :)';
