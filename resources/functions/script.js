    ...
    xhr.then(response => response.json()).then(data => {
        ...
        for (let i=0; i<19; i++) {
            ...
            if (parameterName === 'Wsymb2') {
                ...
                drawVectorArrow(windDirection);

                const syd = "S";

                // Set the wind direction in abbreviation with 2 characters e.g. NW
                if (windDirection >= 337.5 && windDirection < 22.5) {
                    document.getElementById('windDirectionAbreviation').textContent = 'N';
                } else if (windDirection >= 22.5 && windDirection < 67.5) {
                    document.getElementById('windDirectionAbreviation').textContent = 'NO';
                } else if (windDirection >= 67.5 && windDirection < 112.5) {
                    document.getElementById('windDirectionAbreviation').textContent = 'O';
                } else if (windDirection >= 112.5 && windDirection < 157.5) {
                    document.getElementById('windDirectionAbreviation').textContent = 'SO';
                } else if (windDirection >= 157.5 && windDirection < 202.5) {
                    document.getElementById("windDirectionAbreviation").textContent = syd;
                } else if (windDirection >= 202.5 && windDirection < 247.5) {
                    document.getElementById('windDirectionAbreviation').textContent = 'SV';
                } else if (windDirection >= 247.5 && windDirection < 292.5) {
                    document.getElementById('windDirectionAbreviation').textContent = 'V';
                } else if (windDirection >= 292.5 && windDirection < 337.5) {
                    document.getElementById('windDirectionAbreviation').textContent = 'NV';
                };
...
function drawVectorArrow(angle) {
    ...
    // Calculate half of the arrow length
    var halfLength = canvas.height / 2;
  
    // Calculate starting point coordinates
    var startX = centerX - halfLength * Math.cos(angle);
    var startY = centerY - halfLength * Math.sin(angle);
  
    // Calculate endpoint coordinates
    var endX = centerX + halfLength * Math.cos(angle);
    var endY = centerY + halfLength * Math.sin(angle);
  
    // Set the color for the arrow line
    const color = 'white';
    ...
    // Set the color for the arrowhead
    ctx.fillStyle = color;
    ...
    ctx.lineTo(
      endX - arrowSize * Math.cos(angle1),
      endY - arrowSize * Math.sin(angle1)
    );
    ctx.lineTo(
      endX - arrowSize * Math.cos(angle2),
      endY - arrowSize * Math.sin(angle2)
    );
    ctx.closePath();
    ctx.fill();
  };
